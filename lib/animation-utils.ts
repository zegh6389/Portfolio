// Adjusted import to match existing constants file
...
export function throttle<T extends (...args: unknown[]) => void>(
  func: T,
  // Fallback to SCROLL_THROTTLE_MS since PERFORMANCE config differs in constants
  delay: number = ANIMATION_CONFIG.SCROLL_THROTTLE_MS
): T {
...
export function debounce<T extends (...args: unknown[]) => void>(
  func: T,
  // Re-use scroll throttle for debounce default (no explicit resizeDebounce constant present)
  delay: number = ANIMATION_CONFIG.SCROLL_THROTTLE_MS
): T {
  let timeoutId: NodeJS.Timeout | null = null;
...
export function getResponsiveAnimationConfig() {
  if (typeof window === 'undefined') {
    return {
  // Use sensible defaults; constants file doesn't expose performance particle limits
  particleCount: 50,
  ...
  // Mobile heuristic: width < 640 (Tailwind sm breakpoint)
  if (width < 640 || isLowEnd) {
    return {
  particleCount: Math.floor(50 * 0.3),
      enableComplexAnimations: false,
    };
  }

  // Tablet heuristic: width < 1024 (Tailwind lg breakpoint)
  if (width < 1024) {
    return {
  particleCount: Math.floor(50 * 0.6),
      enableComplexAnimations: true,
    };
  }
