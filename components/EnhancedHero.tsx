...
// Removed unused AnimatedText component (not referenced)
...
function ParticleField() {
  ...
  // Reduced particles from 50 to 8 for better performance
  const generateParticles = () => {
    return Array.from({ length: 8 }, (_, i) => {
      // Create deterministic values based on index
      const row = Math.floor(i / 4);
      const col = i % 4;
      const x = (col * 25) + (row % 2 ? 12 : 0); // Spread particles more
      const y = (row * 50) + ((col % 2) * 20); // Better distribution
      const size = ((i % 3) + 2) * 2; // Size cycles through 4, 6, 8
      const duration = 20 + (i % 3) * 5; // Duration cycles through 20, 25, 30 (slower)
      const delay = (i % 4) * 1; // Stagger delays
...
export default function EnhancedHero() {
  ...
  useEffect(() => {
    // Check for battery saver, slow device, etc
